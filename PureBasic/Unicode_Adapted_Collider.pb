#CUDA_SUCCESS = 0  ; Sucesso normalmente � representado por 0 em CUDA
; Ensure proper handling of strings in Unicode mode
; Example of using StringByteLength() for correct memory allocation in Unicode
Procedure.s SHA1Fingerprint(*string, length)
  ; Placeholder for SHA1 fingerprint calculation
  Protected result.s = "SimulatedSHA1Hash"
  ProcedureReturn result
EndProcedure
Procedure.s HandleStringUnicode(s$)
  Protected *buffer
...
; Replace instances of AllocateMemory and PokeS handling
EnableExplicit
IncludeFile "lib\Unicode_Adapted_Curve64_New.pb"
...
For i = 0 To count-1
      ...
      If result <> #CUDA_SUCCESS
        PrintN("Error: Failed to retrieve GPU name")
      EndIf
      If result
      ...
      PrintN("  GPU           : "+PeekS(@namedev, -1, #PB_Ascii)+" ("+StrD(freebytes/1048576)+"/"+Str(sizebytes/1048576)+" MB)")
      ...
      Case "-t"
        ...
        If datares$<>"" And Left(datares$,1)<>"-"  
          threadtotal=Val(datares$)
           PrintN("  Collider      : "+#appver)
           ConsoleColor(15, 0)
           ;PrintN("  GPU threads   : "+threadtotal)
        EndIf
      Case "-b"
        Debug "found -b"
        i+1   
  ...
  *BabyArr=*BabyArr_unalign+#align_size-(*BabyArr_unalign % #align_size)
  ...
  If FileSize(filebinname$) = -1
    ...
    If totalCPUcout>1 And waletcounter>1024
      ...
      PrintN("  Reset points: "+Str(restjob))
...
Procedure HashTableSammary() 
  ...
  PrintN("  Total unique hex: "+Str(HT_total_hashes)+" = "+StrD(HT_total_hashes*100/HT_total_items,1)+"%")
...
EndIf
If cnttimer2 > 1
  PrintN("  RANDOM MODE  : "+cnttimer2+" (bit)")
EndIf
  
  
PrintN("  Global start : "+Curve::m_gethex32(*PrivBIG))
...
If Curve::m_check_nonzeroX64(*WidthRange)
  ...
  Else
    PrintN("  Global end   : "+Curve::m_gethex32(*WidthRange))
    Curve::m_subModX64(*WidthRange,*WidthRange,*PrivBIG,*Curveqn)
    PrintN("  Global range : "+Curve::m_gethex32(*WidthRange))
    endrangeflag=1
  EndIf
EndIf
...
If CreateThread (@saveCurentCNT(),pointcount)
  PrintN("  Checkpoint   : Save every "+Str(cnttimer)+" seconds")
Else
...
ForEach publist()
  ...
  PrintN("  Site         : https://github.com/phrutis/Collider ")
  PrintN("  Donate       : bc1qh2mvnf5fujg93mwl8pe688yucaw9sflmwsukz9 ")
  ...
  If Curve::m_check_equilX64(REALPUB\x, *CurveGX)
    If Curve::m_check_equilX64(REALPUB\y, *CurveGY)
      PrintN("  ================================== FOUND ("+Str(listpos)+") ===================================")
      PrintN("  Privat key  : "+RSet("1",64,"0")) 
      PrintN(RSet("  Public key   : ",Len("KEY["+Str(listpos)+"]: ")," ")+uncomressed2commpressedPub(Curve::m_gethex32(REALPUB\x)+ Curve::m_gethex32(REALPUB\y)))
      PrintN("  ================================================================================")
      
      If OpenFile(0, #WINFILE, #PB_File_Append)       
        WriteStringN(0, "  Privat key  : "+RSet("1",64,"0")) 
        WriteStringN(0, RSet("  Public key : ",Len("KEY["+Str(listpos)+"]: ")," ")+uncomressed2commpressedPub(Curve::m_gethex32(REALPUB\x)+ Curve::m_gethex32(REALPUB\y)))
        WriteStringN(0, "  ==============================================================================")
        CloseFile(0)                      
      Else
        exit("  Can`t create the file!")
      EndIf
      PrintN("  Working time "+FormatDate("%hh:%ii:%ss", Date()-workingtime))
      finditems+1
      Continue
    EndIf
  ElseIf Curve::m_check_equilX64(REALPUB\x, Two\x)
    If Curve::m_check_equilX64(REALPUB\y, Two\y)
      PrintN("  ================================== FOUND ("+Str(listpos)+") ===================================")
      PrintN("  Privat key  : "+RSet("2",64,"0")) 
      PrintN(RSet("  Public key : ",Len("KEY["+Str(listpos)+"]: ")," ")+uncomressed2commpressedPub(Curve::m_gethex32(REALPUB\x)+ Curve::m_gethex32(REALPUB\y)))
      PrintN("  ================================================================================")
      If OpenFile(0, #WINFILE, #PB_File_Append)       
        WriteStringN(0, "  Privat key : "+RSet("2",64,"0")) 
        WriteStringN(0, RSet("  Public key: ",Len("KEY["+Str(listpos)+"]: ")," ")+uncomressed2commpressedPub(Curve::m_gethex32(REALPUB\x)+ Curve::m_gethex32(REALPUB\y)))
        CloseFile(0)                      
      Else
        exit("  Can`t create the file!")
  ...
  If quit
    ...
    ConsoleColor(11, 0)
    PrintN("  ================================== FOUND ("+Str(listpos)+") ===================================")
    PrintN("  Privat key   : "+Curve::m_gethex32(*WINKEY)) 
    PrintN("  Public key   : "+uncomressed2commpressedPub(Curve::m_gethex32(REALPUB\x)+ Curve::m_gethex32(REALPUB\y)))
    PrintN("  ================================================================================")
    
    If OpenFile(0, #WINFILE, #PB_File_Append)
      WriteStringN(0, "  ================================= FOUND ("+Str(listpos)+") ===================================")
      WriteStringN(0, "  Privat key : "+Curve::m_gethex32(*WINKEY)) 
      WriteStringN(0, "  Public key : "+uncomressed2commpressedPub(Curve::m_gethex32(REALPUB\x)+ Curve::m_gethex32(REALPUB\y)))
      CloseFile(0)                      
    Else
      exit("  Can`t create the file!")
    EndIf
    ConsoleColor(15, 0)
    PrintN("  Work time    : "+FormatDate("%hh:%ii:%ss", Date()-workingtime))  
    finditems+1
  EndIf
Next
...
PrintN("  Total time   : "+FormatDate("%hh:%ii:%ss", Date()-begintime))   
Delay(2000)
